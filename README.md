...
O lexer utiliza regex POSIX pré-compiladas e uma tabela declarativa de tokens (`TokenDefinition`), seguindo a ordem de prioridade abaixo em cada chamada de `next_token`:

1. **Frases multi-palavra** (`handle_complex_tokens`) — casadas por `strncmp` com word boundary check
2. **Números** (`read_number`) — regex `^[0-9]+\.[0-9]+` e `^[0-9]+`
3. **Strings** (`read_string`) — regex `^"[^"]*"`
4. **Identificadores e palavras-chave** (`read_identifier`) — regex `^[a-zA-Z_][a-zA-Z0-9_]*`, seguida de lookup na tabela

As regexes são compiladas uma única vez em `init_lexer` e liberadas em `del_lexer`.
